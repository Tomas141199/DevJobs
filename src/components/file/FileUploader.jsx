...
const CVUploader = ({ setUrlFile }) => {
  const [upload, setUpload] = useState(0);

  async function handleChange(file, setUpload) {
    //Archivo a subir
    ...
    try {
      const storageRef = ref(storage, "cv/" + imagenfile.name);
  ...
  return (
    <FileUploader
      multiple={false}
      handleChange={(file) => handleChange(file, setUpload)}
      maxSize={10}
      name="file"
      types={fileTypes}
    />
