//Form validation and schemas
import { Form, Formik } from "formik";
import Button from "../../components/form/Button";
import { publicacionSchema } from "./../../validation/publicacion";
//Components
import FieldWithLabel from "./../../components/form/FieldWithLabel";
import Select from "./../../components/form/Select";
import { useState, useEffect } from "react";
import GoogleMap from "../../components/maps/GoogleMap";
import TextField from "../../components/form/TrixEditor";
import NavAdmin from "../../components/ui/NavAdmin";
import Fade from "react-reveal/Fade";
//Form data
import ListSkills from "./../../components/form/ListSkills";
import ImageUploader from "./../../components/file/ImageUploader";
import useCollection from "./../../hooks/useCollection";
import fireService from "./../../firebase/firebaseservice";
//Notify
import { useContext } from "react";
import NotifyContext from "./../../context/notify/notifyContext";
import { errorNotify } from "./../../helpers/notify";
import { useNavigate } from "react-router-dom";
import AuthContext from "./../../context/auth/AuthContext";
import { useParams } from "react-router-dom";
import Spin from "./../../components/spinners/Spin";
import ListSkillsEdit from "../../components/form/ListSkillListEdit";

const EditarVacante = () => {
  const { id } = useParams();
  const [vacante, setVacante] = useState([]);
  const [loading, setLoading] = useState(false);
  //Notificacion
  const navigate = useNavigate();
  const { mostrarNotificacion } = useContext(NotifyContext);
  //Valores para los inputs
  const categorias = useCollection("Categorias");
  const experiencias = useCollection("Experiencias");
  const salarios = useCollection("Salarios");
  //Valores para el formulario
  const [imagen, setUrlImagen] = useState("");
  const [listaHabilidades, setSkillList] = useState("");
  const [direccion, setAddress] = useState("");
  const [mapa, setMapCenter] = useState({});

  useEffect(() => {
    setLoading(true);
    const getVacante = async () => {
      const data = await fireService.getVacanteById(id);
      setVacante(data);
      setAddress(data.address);
      setMapCenter(data.mapCenter);
      setUrlImagen(data.urlImagen);
      setLoading(false);
    };
    getVacante();
  }, []);

  const { skillList, address, mapCenter } = vacante;

  const handleSubmit = (values) => {
    //Extracion de los atributos para crear el objeto
    const { titulo, categoria, experiencia, salario, descripcion, empresa } =
      values;

    if (direccion === "") {
      errorNotify("Agrega una ubicacion");
      return;
    }

    if (imagen == "") {
      errorNotify("Agrega una imagen");
      return;
    }

    if (listaHabilidades == "") {
      errorNotify("Escoge almenos 3 habilidades");
      return;
    }

    const vacante = {
      titulo,
      categoria,
      experiencia,
      salario,
      descripcion,
      empresa,
      listaHabilidades,
      imagen,
      direccion,
      mapa,
    };

    try {
      fireService.editVacante(id, vacante);
      mostrarNotificacion(
        "Cambios guardados!, tu publicacion ya cuenta con los nuevos cambios"
      );
      navigate("/");
    } catch (error) {
      errorNotify(error.code);
    }
  };

  return (
    <>
      <NavAdmin />
      <Fade big>
        {loading ? (
          <Spin />
        ) : (
          <div className="w-10/12 container bg-gray-300 py-4 px-1 mx-auto mt-10">
            <h1 className="text-center text-blue-deep font-semibold text-2xl">
              Nueva <span className="text-primary-jade">Vacante</span>
            </h1>
            <Formik
              initialValues={{
                titulo: vacante.titulo ?? "",
                descripcion: vacante.descripcion ?? "",
                categoria: vacante.categoria ?? "",
                empresa: vacante.empresa ?? "",
                experiencia: vacante.experiencia ?? "",
                salario: vacante.salario ?? "",
              }}
              enableReinitialize={true}
              validationSchema={publicacionSchema}
              onSubmit={handleSubmit}
            >
              <Form className="mt-8 w-full sm:w-11/12 mx-auto">
                <FieldWithLabel
                  name="titulo"
                  type="text"
                  placeholder="Titulo de la vacante"
                  label="Titulo"
                />
                <FieldWithLabel
                  name="empresa"
                  type="text"
                  placeholder="Empresa que oferta"
                  label="Nombre de la empresa"
                />
                <Select
                  values={categorias}
                  name="categoria"
                  label="Categoria"
                />
                <Select
                  values={experiencias}
                  name="experiencia"
                  label="Experiencia"
                />
                <Select values={salarios} name="salario" label="Salario" />

                <div className="mb-8 h-10">
                  <label className="text-xs">Imagen vacante</label>
                  <ImageUploader setUrlImagen={setUrlImagen} />
                </div>

                <ListSkillsEdit
                  setSkillList={setSkillList}
                  oldskills={skillList}
                />

                <TextField />

                <label className="text-xs mt-3">Ubicacion</label>
                {/* Mapa */}
                <GoogleMap
                  address={direccion}
                  setAddress={setAddress}
                  mapCenter={mapCenter}
                  setMapCenter={setMapCenter}
                />

                {/* Submit para enviar formulario */}
                <Button value="Publicar" color="bg-primary-jade" />
              </Form>
            </Formik>
          </div>
        )}
      </Fade>
    </>
  );
};

export default EditarVacante;
